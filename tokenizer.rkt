...
(define (String_Token pos val tokens)
  (if (< pos end)
    ...
    (if (> (string-length val) 0) (append tokens (list (string_Token val))) tokens)))
    ...
    (let ([i (item pos)])
      (if (is_letter i)
          ...
          (if (> (string-length val) 0)
              (Period_Token pos (append tokens (list (identifier_Token val))))
              (Period_Token pos tokens))))
    (if (> (string-length val) 0)
        (append tokens (list (identifier_Token val)))
        tokens)))
...
(define (Period_Token pos tokens)
  (if (< pos end)
      (let ([i (item pos)])
        (if (is_period i)
            (Identifier_Token (add1 pos) "" (append tokens (list (period_Token #t))))
...
(define (Operator_Token pos val tokens)
  (if (< pos end)
      ...
      (if (> (string-length val) 0) (append tokens (list (operator_Token val))) tokens)))
